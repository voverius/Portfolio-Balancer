...
def GetFF(tadict, periods):        # FOURIER FUNCTION COEFFICIENTS

    """
    :param tadict:      Dictionary with OHLC & TAs
    :param periods:     Array of periods

    :return:            A dictionary with periods as keys with FF numpy matrices

    Description         Fourier Function - converts signal to sin&cos function - Fourier Transform
    """

    plot = False
    results = {}
    prices = tadict['prices']
    detrended = tadict['detrended'][:, 3, 1]

    for period in periods:

        coeffs = np.zeros(((len(prices) - period + 1), 4))
        xt = np.linspace(0, period, period * 10)

        for i in range(0, len(coeffs)):

            x = np.arange(0, period)
            y = detrended[i:(i + period)]

            try:
                temp, _ = curve_fit(fseries, x, y)
                flag = True
